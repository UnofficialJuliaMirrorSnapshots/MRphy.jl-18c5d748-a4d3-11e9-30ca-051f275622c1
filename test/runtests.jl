...
spa = mSpinArray(trues((3,1)); γ=γ¹H, M=M)
...
loc, Δf, b1Map = [zeros((3,2)) ones((3,1))]u"cm", 0u"Hz", 1

Mo, _ = blochSim(spa, p, loc; Δf=Δf, b1Map=b1Map, doHist=false)

@testset "blochSim tests" for _ = [1] # setting _ = [] shuts down this testset

  Beff = cat(Pulse2B(p, loc; Δf=Δf, b1Map=b1Map, γ=spa.γ)...; dims=3)
  @test Beff == cat(Pulse2B(p.rf, p.gr, loc; Δf=Δf, b1Map=b1Map, γ=spa.γ)...;
                    dims=3)

  A, B = B2AB(Beff; T1=spa.T1, T2=spa.T2, γ=spa.γ, dt=p.dt)

  _, Mhst = blochSim(M, Beff; T1=spa.T1,T2=spa.T2,γ=spa.γ,dt=p.dt,doHist=true);
  MoAB = blochSim(M, A, B)
